(() => {
  ...
  const W=canvas.width,H=canvas.height,SCALE=H/270,FOV=Math.PI/3,MAX=32,MONSTER_SPEED_SCALE=.7;
  ...
  function drawAisleMarkings(){
    const paths=player.floor===0?[[1.5,8.8,20,8.8],[7.2,1.4,7.2,16.2],[15.2,1.4,15.2,16.2]]:[[1.5,8.8,20,8.8],[7.2,1.4,7.2,16.2],[16.2,1.4,16.2,16.2]];
    ctx.save();ctx.strokeStyle='rgba(185,151,54,.22)';ctx.lineWidth=2;ctx.setLineDash([14,12]);
    for(const [x1,y1,x2,y2] of paths){const a=screenPoint(x1,y1),b=screenPoint(x2,y2);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke()}
    ctx.setLineDash([]);ctx.fillStyle='rgba(190,157,55,.18)';
    for(const [x,y,rotation] of player.floor===0?[[5.4,8.8,0],[13.5,8.8,0],[15.2,5.1,Math.PI/2],[15.2,12.7,Math.PI/2]]:[[5.4,8.8,0],[14.2,8.8,0],[16.2,5.1,Math.PI/2],[16.2,12.7,Math.PI/2]]){const p=screenPoint(x,y);ctx.save();ctx.translate(p.x,p.y);ctx.rotate(rotation);ctx.beginPath();ctx.moveTo(-10,-5);ctx.lineTo(10,0);ctx.lineTo(-10,5);ctx.closePath();ctx.fill();ctx.restore()}
    ctx.restore();
  }
  ...
  function renderDarkness2D(px,py){
    const l=lightCtx;l.clearRect(0,0,W,H);l.fillStyle=state.flash?'rgba(0,3,4,.76)':'rgba(0,2,3,.9)';l.fillRect(0,0,W,H);l.globalCompositeOperation='destination-out';
    for(const lamp of industrialLights.filter(lamp=>lamp.floor===player.floor)){const p=screenPoint(lamp.x,lamp.y),radius=lamp.radius*TILE,glow=l.createRadialGradient(p.x,p.y,0,p.x,p.y,radius);glow.addColorStop(0,'rgba(0,0,0,.28)');glow.addColorStop(.45,'rgba(0,0,0,.13)');glow.addColorStop(1,'rgba(0,0,0,0)');l.fillStyle=glow;l.fillRect(p.x-radius,p.y-radius,radius*2,radius*2)}
    let ambient=l.createRadialGradient(px,py,10,px,py,state.flash?100:52);ambient.addColorStop(0,'rgba(0,0,0,.95)');ambient.addColorStop(1,'rgba(0,0,0,0)');l.fillStyle=ambient;l.fillRect(px-110,py-110,220,220);if(state.flash&&state.battery>0){l.save();l.beginPath();l.moveTo(px,py);for(let i=0;i<=48;i++){const a=player.a-.5+i/48,r=castRay(a).d*TILE;l.lineTo(px+Math.cos(a)*r,py+Math.sin(a)*r)}l.closePath();l.clip();const cone=l.createRadialGradient(px,py,20,px,py,420);cone.addColorStop(0,'rgba(0,0,0,.98)');cone.addColorStop(.55,'rgba(0,0,0,.72)');cone.addColorStop(1,'rgba(0,0,0,0)');l.fillStyle=cone;l.fillRect(0,0,W,H);l.restore()}l.globalCompositeOperation='source-over';ctx.drawImage(lightCanvas,0,0);
  }
  function render2D(){
